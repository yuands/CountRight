    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                    ...
                    with torch.enable_grad():
                        ...
                        for latent, index, curr_prompt_embeds, curr_add_text_embeds, curr_add_time_ids in \
                            ...
                            if i in thresholds.keys() and loss > thresholds[i]:
                                print(f"Iteration {i} | Entering refinement | Initial Loss: {loss:0.4f} | Threshold: {thresholds[i]:0.4f}")
                ...
                # ── 步级潜空间分区植入 ──────────────────────────────────────
                # step_latent_noise_injection > 0 时，在 step 0 ~ N-1 的每一步结束后
                # 对当前 latents 施加空间分区先验（坑内保留、坑外衰减）。
                # 与 step_latent_noise_injection=0（仅在管线开始前施加一次）互斥。
                if (step_latent_noise_injection > 0
                        and i < step_latent_noise_injection
                        and self.desired_mask is not None):
                    latents = apply_latent_partition(latents, self.desired_mask)
                    print(f"[StepLatentInjection] step {i}: 分区植入完成")
